...
public class ScenManagerLevel9 : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (Backgroundlayers != null && BackGroundSurface != null && HeroPrefab != null && BatPrefab != null && CheckptPrefab != null && NextlvlWay != null && EntryWay != null)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Scientist S = Instantiate<Scientist>(Scientistpf, new Vector3((i + 1) * -25.4f, -3.9f, 0f), Quaternion.identity);
            }
        ...
        if (Zombies.Count > 0)
        {
            ...
            for (int x = 1; x < 4; x++)
            {
                ...
                for (int i = 0; i < no_of_zombies; i++)
                {
                    ...
                    newZombie.transform.localPosition = new Vector2((x - 1) * k - 24.35f + i * j, Zombies[i].transform.localPosition.y - .2f);
